...
export async function getAdminOrders(req: AuthRequest, res: Response) {
  try {
    ...
    // Sorting
    const sortColumn = ["created_at", "total", "status"].includes(sort)
      ? sort
      : "created_at";
...
export async function getOrderStats(_req: AuthRequest, res: Response) {
  try {
    ...
    const lastMonthOrders = revenueOrders.filter((o: any) => {
...
export async function updateShippingSettings(req: AuthRequest, res: Response) {
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("shipping_settings")
      .upsert(
        { ...payload, id: "default-shipping-settings" },
        {
          onConflict: "id",
...
export async function getAdminAnalytics(req: AuthRequest, res: Response) {
  try {
    ...
    > = {};
    const scentFamilyCounts: Record<
      string,
      { family: string; qty: number; revenue: number }
    > = {};
    ...
    const scentFamilies = Object.values(scentFamilyCounts).sort(
      (a, b) => b.qty - a.qty,
    );
    ...
    res.json({
      summary: {
        ...
        totalDiscount,
        pendingOrders:
          (statusDistribution["pending_payment"] || 0) +
