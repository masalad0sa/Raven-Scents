...
export async function createPaymentOrder(req: AuthRequest, res: Response) {
  try {
    ...
    const { order_id, total } = rpcResult as { order_id: string; total: number };
...
export async function verifyPayment(req: AuthRequest, res: Response) {
  try {
    ...
    // 2. Mark the order as confirmed and store the Razorpay payment ID
...
export async function cancelOrder(req: AuthRequest, res: Response) {
  try {
    ...
    const { error } = await supabaseAdmin.rpc('cancel_order_and_restore_stock', {
      p_order_id: order_id,
      p_user_id:  req.userId ?? null,
    });
...
export async function handleRazorpayWebhook(req: AuthRequest, res: Response) {
  try {
    ...
    if (event === 'order.paid' || event === 'payment.captured') {
      ...
      const orderId = paymentEntity?.notes?.order_id || req.body.payload?.order?.entity?.receipt;
