...
export default function Shop() {
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedGender.length > 0) result = result.filter(p => selectedGender.includes(p.gender));
    if (selectedScentFamilies.length > 0) result = result.filter(p => selectedScentFamilies.includes(p.scentFamily));
    ...
    return result;
  }, [search, selectedGender, selectedScentFamilies, priceMax, sort, showNew, showBestseller]);
  ...
  const hasFilters = selectedGender.length > 0 || selectedScentFamilies.length > 0 || priceMax < 50000 || showNew || showBestseller || search.trim().length > 0;
  ...
  const filterSection = (title: string, children: React.ReactNode) => (
    <div style={{ marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid #EDE8DC' }}>
      <h4 style={{ fontFamily: 'var(--font-display)', fontSize: '0.62rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--color-muted)', marginBottom: '0.75rem' }}>{title}</h4>
      {children}
    </div>
  );
        ...
        <div style={{ borderBottom: '1px solid rgba(212,175,55,0.2)', background: 'var(--color-surface)', padding: '2rem 0' }}>
          <div className="container">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-gold)', marginBottom: '0.5rem' }}>Discover</p>
              <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 300, color: 'var(--color-primary)' }}>All Fragrances</h1>
            </motion.div>
          </div>
        </div>
        ...
        <div className="container" style={{ padding: '2rem 2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: '1 1 260px', maxWidth: 360 }}>
              ...
              <input type="text" placeholder="Search fragrances..." value={search} onChange={e => setSearch(e.target.value)} className="input" style={{ paddingLeft: '2.25rem' }} />
            </div>
            <select value={sort} onChange={e => setSort(e.target.value as SortOption)} className="input" style={{ width: 'auto', cursor: 'pointer' }}>
            ...
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="btn btn-outline" style={{ gap: '0.5rem', padding: '0.75rem 1.25rem' }}>
            ...
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.7rem', letterSpacing: '0.08em', color: 'var(--color-muted)', marginLeft: 'auto' }}>{filtered.length} RESULTS</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start' }}>
            ...
            {sidebarOpen && (
              <motion.aside
                ...
                style={{ width: 240, flexShrink: 0, background: '#fff', borderRadius: 6, padding: '1.5rem', border: '1px solid #EDE8DC', position: 'sticky', top: 92, maxHeight: 'calc(100vh - 110px)', overflowY: 'auto' }}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                  ...
                  {hasFilters && <button onClick={clearAll} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-gold)', fontFamily: 'var(--font-display)', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Clear All</button>}
                </div>

                {/* Price — at top */}
                {filterSection('Max Price', (
                  <>
                    <input type="range" min={0} max={50000} step={500} value={priceMax} onChange={e => setPriceMax(Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--color-gold)', display: 'block', marginBottom: '0.5rem' }} />
                ...
                {filterSection('Gender', (
                  <>
                    {['masculine', 'feminine', 'unisex'].map(g => (
                      <label key={g} style={checkStyle(selectedGender.includes(g))}>
                ...
                <div>
                  ...
                  {scentFamilies.map(sf => (
                    <label key={sf} style={checkStyle(selectedScentFamilies.includes(sf))}>
