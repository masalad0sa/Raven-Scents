...
export default function OrderConfirmation() {
  const orderId = 'LUX' + Math.random().toString(36).substr(2, 9).toUpperCase();
    ...
    <>
      ...
      <main style={{ paddingTop: 72, background: 'var(--color-ivory)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', padding: '4rem 2rem', maxWidth: 560 }}>
          ...
          >
            <Check size={36} strokeWidth={2.5} color="#1A0A2E" />
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 300, color: 'var(--color-primary)', lineHeight: 1.1, marginBottom: '1rem' }}>
            ...
            {/* Order Details */}
            <div style={{ background: '#fff', border: '1px solid #EDE8DC', borderRadius: 8, padding: '1.5rem', textAlign: 'left', marginBottom: '2rem' }}>
              {[
                { label: 'Order ID', value: orderId },
                { label: 'Estimated Delivery', value: '5–7 business days' },
                { label: 'Shipping Method', value: 'Express Delivery' },
              ].map(row => (
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid #F5F0E8' }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', fontWeight: 600, color: 'var(--color-primary)' }}>{row.value}</span>
                </div>
              ))}
            </div>
              ...
              {[
                ...
                <div key={i} style={{ textAlign: 'center' }}>
                  <div style={{
                    ...
                    background: s.done ? 'var(--color-gold)' : '#EDE8DC',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    margin: '0 auto 0.5rem',
                    fontFamily: 'var(--font-display)', fontSize: '0.7rem',
                    color: s.done ? 'var(--color-primary)' : '#B0A090',
                    fontWeight: 700,
                  }}>
                    {typeof s.icon === 'string' ? s.icon : <Package size={14} />}
                  </div>
                  <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.58rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: s.done ? 'var(--color-primary)' : 'var(--color-muted)' }}>
                    {s.label}
                  </p>
                </div>
              ))}
