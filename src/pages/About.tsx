...
const timeline = [
  { year: '2018', title: 'The Beginning', desc: 'Founded in a small studio with a single reed diffuser and a dream. Our first fragrance — Velvet Rose — was born here.' },
  { year: '2020', title: 'First Collection', desc: 'Launched 6 fragrances under our inaugural "Origins" collection, selling out within 3 weeks.' },
  { year: '2022', title: 'Our Own Lab', desc: 'Opened a dedicated fragrance lab allowing us full control over every note, every molecule, every memory.' },
  { year: '2024', title: 'LuxeScent Evolves', desc: 'Expanded to 50+ fragrances, launched nationwide shipping, and introduced our signature Oud Bouquet series.' },
];
    ...
    <>
      ...
      <main style={{ paddingTop: 72, background: 'var(--color-ivory)' }}>
        ...
        <section style={{ position: 'relative', minHeight: '70vh', display: 'flex', alignItems: 'center', background: 'var(--color-primary)', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', left: '-100px', bottom: '-200px', opacity: 0.2, pointerEvents: 'none' }}>
            <GradientBlob size={700} reactToMouse={false} />
          </div>
          <div style={{ position: 'absolute', right: '-50px', top: '-150px', opacity: 0.12, pointerEvents: 'none' }}>
          ...
          <div className="container" style={{ position: 'relative', zIndex: 1, padding: '6rem 2rem' }}>
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2.5rem, 7vw, 5.5rem)', fontWeight: 300, color: 'var(--color-ivory)', lineHeight: 1.0, marginBottom: '1.75rem' }}>
                We Don't Follow<br /><em style={{ color: 'var(--color-gold)' }}>Trends.</em><br />We Set Them.
              </h1>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '1rem', color: 'rgba(250,246,238,0.65)', maxWidth: 520, lineHeight: 1.9 }}>
                LuxeScent is an independent perfume house — crafting bold, intimate, and unforgettable fragrances entirely from our own studio. No celebrities, no licensing. Just perfectly made scents.
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="section" style={{ background: 'var(--color-ivory)' }}>
          <div className="container">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '4rem' }}>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-gold)', marginBottom: '0.75rem' }}>Our Journey</p>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 300, color: 'var(--color-primary)' }}>From Studio to Shelf</h2>
            </motion.div>

            <div style={{ position: 'relative', maxWidth: 800, margin: '0 auto' }}>
              {/* Vertical line */}
              <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: 1, background: 'linear-gradient(to bottom, transparent, var(--color-gold), transparent)', transform: 'translateX(-50%)' }} />

              {timeline.map((item, i) => (
                <motion.div key={item.year} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
                  style={{ display: 'flex', gap: '3rem', alignItems: 'flex-start', marginBottom: '3rem', flexDirection: i % 2 === 0 ? 'row' : 'row-reverse' }}>
                  <div style={{ flex: 1, textAlign: i % 2 === 0 ? 'right' : 'left' }}>
                    <span style={{ fontFamily: 'var(--font-serif)', fontSize: '2.5rem', fontWeight: 300, color: 'rgba(26,10,46,0.1)', display: 'block', lineHeight: 1 }}>{item.year}</span>
                    <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.25rem', fontWeight: 500, color: 'var(--color-primary)', marginBottom: '0.5rem' }}>{item.title}</h3>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.875rem', color: 'var(--color-muted)', lineHeight: 1.8 }}>{item.desc}</p>
                  </div>
                  {/* Center dot */}
                  <div style={{ width: 16, height: 16, borderRadius: '50%', background: 'var(--color-gold)', flexShrink: 0, marginTop: '2.5rem', boxShadow: '0 0 0 4px rgba(212,175,55,0.2)' }} />
                  <div style={{ flex: 1 }} />
                </motion.div>
              ))}
            </div>
        ...
        <section className="section" style={{ background: 'var(--color-surface)' }}>
          <div className="container">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '4rem' }}>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-gold)', marginBottom: '0.75rem' }}>What Drives Us</p>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 300, color: 'var(--color-primary)' }}>Built on These Principles</h2>
            </motion.div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.5rem' }}>
              {values.map(({ Icon, title, desc }, i) => (
                <motion.div key={title} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
                  style={{ padding: '2.25rem', background: '#fff', borderRadius: 8, border: '1px solid #EDE8DC', transition: 'box-shadow 0.3s, transform 0.3s' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 20px 48px rgba(26,10,46,0.1)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = 'none'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; }}
                >
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: 'rgba(212,175,55,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.25rem' }}>
                    <Icon size={22} color="var(--color-gold)" strokeWidth={1.5} />
                  </div>
                  <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.2rem', fontWeight: 500, color: 'var(--color-primary)', marginBottom: '0.75rem' }}>{title}</h3>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.875rem', color: 'var(--color-muted)', lineHeight: 1.85 }}>{desc}</p>
                </motion.div>
              ))}
            </div>
          </div>
                ...
                {[
                  ...
                  <div key={item.step} style={{ display: 'flex', gap: '1.25rem', marginBottom: '1.5rem' }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', color: 'rgba(250,246,238,0.55)', lineHeight: 1.75 }}>{item.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section className="section" style={{ background: 'var(--color-ivory)', textAlign: 'center' }}>
          <div className="container" style={{ maxWidth: 600 }}>
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 300, color: 'var(--color-primary)', lineHeight: 1.2, marginBottom: '1.5rem' }}>
