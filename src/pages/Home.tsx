...
const fadeUp = {
  ...
  visible: (i = 0) => ({ opacity: 1, y: 0, transition: { delay: i * 0.12, duration: 0.65, ease: [0.25, 0.46, 0.45, 0.94] } }),
};
    ...
    <section style={{
      ...
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
        {[800, 550, 300].map((size, i) => (
          <div key={i} className="hud-circle" style={{
            width: size,
            height: size,
            top: '50%',
            left: '50%',
      ...
      {/* Content */}
      <div className="container" style={{ position: 'relative', zIndex: 3, padding: '8rem 2rem 4rem' }}>
        {/* Small label */}
        <motion.p
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3rem', flexWrap: 'wrap' }}>
          ...
          >
            A curation of rare, niche, and luxury fragrances for the discerning nose. From fresh citrus to deep oud.
          </motion.p>
          <motion.div
        ...
        >
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.55rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-muted)' }}>
            Scroll
          </span>
          <style>{`
      ...
      <div style={{
        ...
        left: 0,
        right: 0,
        borderTop: '1px solid rgba(26,10,46,0.08)',
        background: 'rgba(250, 246, 238, 0.85)',
        backdropFilter: 'blur(12px)',
        padding: '1.25rem 2rem',
        zIndex: 3,
      }}>
        <div className="container" style={{ display: 'flex', gap: '3rem', overflow: 'auto' }} className="no-scrollbar">
          {[
            { value: '200+', label: 'Luxury Fragrances' },
...
function CollectionCard({ title, subtitle, image, link }: { title: string; subtitle: string; image: string; link: string }) {
  return (
    <Link to={link} style={{ textDecoration: 'none', display: 'block', position: 'relative', aspectRatio: '3/4', overflow: 'hidden', borderRadius: 6, background: '#111' }}>
      <img src={image} alt={title} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.75, transition: 'transform 0.6s ease, opacity 0.4s ease' }}
      ...
      <main>
        ...
        <section className="section" style={{ background: 'var(--color-surface)' }}>
          <div className="container">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ marginBottom: '3rem', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1.25rem', marginBottom: '3rem' }}>
              <CollectionCard
                ...
                link="/shop?gender=feminine&scentFamily=Floral"
              />
              <CollectionCard
                title="Dark & Woody"
                subtitle="For Him"
                image="https://images.unsplash.com/photo-1547887537-6158d64c35b3?w=600&q=80"
                link="/shop?gender=masculine"
              />
              <CollectionCard
                title="Oriental Oud"
                subtitle="Unisex"
                image="https://images.unsplash.com/photo-1588776814546-1ffbb172c5e4?w=600&q=80"
                link="/shop?scentFamily=Woody+Oriental"
              />
              <CollectionCard
                title="Fresh Aquatics"
                subtitle="Summer Picks"
                image="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&q=80"
                link="/shop?scentFamily=Aquatic"
              />
            </div>
        ...
        <section className="section" style={{ background: 'var(--color-primary)', position: 'relative', overflow: 'hidden' }}>
          {/* Decorative blob */}
          <div style={{ position: 'absolute', right: '-200px', top: '-200px', opacity: 0.15 }}>
          ...
          <div className="container" style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'center' }}>
              <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
                ...
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.95rem', color: 'rgba(250,246,238,0.65)', lineHeight: 1.9, marginBottom: '2rem' }}>
                  We believe every scent tells a story. From the ancient perfumery traditions of the Arabian Peninsula to the floral gardens of Grasse, LuxeScent curates fragrances that transcend time — turning ordinary moments into extraordinary memories.
                </p>
                <div style={{ display: 'flex', gap: '2.5rem', marginBottom: '2.5rem' }}>
                  {[{ v: '1921', l: 'Year of Chanel №5' }, { v: '15+', l: 'Premium Brands' }, { v: 'Global', l: 'Sourcing' }].map(s => (
                    <div key={s.l}>
                      <div style={{ fontFamily: 'var(--font-serif)', fontSize: '1.75rem', color: 'var(--color-gold)' }}>{s.v}</div>
                      <div style={{ fontFamily: 'var(--font-display)', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'rgba(250,246,238,0.5)', marginTop: '0.25rem' }}>{s.l}</div>
                    </div>
                  ))}
                </div>
                <Link to="/about" className="btn btn-gold">Our Story →</Link>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                {[
          ...
          <style>{`
            @media (max-width: 900px) {
              ...
              .brand-story-images { display: none !important; }
            }
          `}</style>
        </section>
        ...
        {/* Scent Families Banner */}
        <section style={{ background: 'var(--color-surface)', padding: '3rem 0', overflow: 'hidden' }}>
          <div style={{ display: 'flex', gap: '3rem', animation: 'marquee 20s linear infinite', whiteSpace: 'nowrap', width: 'max-content' }}>
            {[...Array(3)].flatMap(() =>
        ...
        <section className="section" style={{ background: 'var(--color-ivory)' }}>
          <div className="container" style={{ maxWidth: 700, textAlign: 'center' }}>
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--color-gold)', marginBottom: '1rem' }}>Stay Inspired</p>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 300, color: 'var(--color-primary)', lineHeight: 1.2, marginBottom: '1rem' }}>
                The Scent Letter
              </h2>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.95rem', color: 'var(--color-muted)', lineHeight: 1.8, marginBottom: '2.5rem' }}>
                New arrivals, expert fragrance guides, and exclusive subscriber offers — delivered monthly.
              </p>
              <div style={{ display: 'flex', gap: '0.75rem', maxWidth: 480, margin: '0 auto' }}>
                <input
                  type="email"
                  ...
                  style={{ flex: 1 }}
                />
                <button className="btn btn-gold" style={{ whiteSpace: 'nowrap' }}>Subscribe</button>
              </div>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.72rem', color: '#B0A090', marginTop: '1rem' }}>
