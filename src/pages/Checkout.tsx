...
export default function Checkout() {
  ...
  const [shipping, setShipping] = useState<ShippingData>({ firstName: '', lastName: '', email: '', phone: '', address: '', city: '', state: '', pincode: '' });
  const [payment, setPayment] = useState<PaymentData>({ cardName: '', cardNumber: '', expiry: '', cvv: '' });
  ...
  const validateShipping = () => {
    ...
    if (!shipping.phone.match(/^[0-9]{10}$/)) e.phone = '10-digit number required';
    if (!shipping.address.trim()) e.address = 'Required';
    if (!shipping.city.trim()) e.city = 'Required';
    if (!shipping.state) e.state = 'Required';
    if (!shipping.pincode.match(/^[0-9]{6}$/)) e.pincode = '6-digit pincode required';
    setErrors(e);
  ...
  const validatePayment = () => {
    ...
    if (!payment.cardNumber.replace(/\s/g, '').match(/^[0-9]{16}$/)) e.cardNumber = 'Valid 16-digit card required';
    if (!payment.expiry.match(/^(0[1-9]|1[0-2])\/[0-9]{2}$/)) e.expiry = 'MM/YY format required';
    if (!payment.cvv.match(/^[0-9]{3,4}$/)) e.cvv = '3-4 digits required';
    setErrors(e);
    return Object.keys(e).length === 0;
  };

  const formatCard = (value: string) => value.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim().slice(0, 19);
  ...
  const inputGroup = (label: string, field: string, value: string, onChange: (v: string) => void, opts?: { type?: string; placeholder?: string; half?: boolean }) => (
    <div style={{ flex: opts?.half ? '1 1 calc(50% - 0.5rem)' : '1 1 100%', minWidth: opts?.half ? 120 : undefined }}>
      <label style={{ fontFamily: 'var(--font-display)', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-muted)', display: 'block', marginBottom: '0.4rem' }}>{label}</label>
      <input
        type={opts?.type ?? 'text'}
        value={value}
        ...
        className="input"
        style={{ borderColor: errors[field] ? 'var(--color-error)' : undefined }}
      />
      {errors[field] && <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.72rem', color: 'var(--color-error)', marginTop: '0.25rem' }}>{errors[field]}</p>}
    </div>
  );
      ...
      <main style={{ paddingTop: 72, background: 'var(--color-ivory)', minHeight: '100vh' }}>
        <div style={{ background: 'var(--color-surface)', padding: '2rem 0', borderBottom: '1px solid rgba(212,175,55,0.2)' }}>
          <div className="container">
            <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem, 4vw, 2.5rem)', fontWeight: 300, color: 'var(--color-primary)', marginBottom: '1.5rem' }}>Checkout</h1>
            {/* Step Indicator */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '0' }}>
              {STEPS.map((s, i) => (
                <div key={s} style={{ display: 'flex', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                    <div className={`step-dot ${i < currentStepIndex ? 'done' : i === currentStepIndex ? 'active' : 'inactive'}`}>
                      {i < currentStepIndex ? <Check size={12} /> : i + 1}
                    </div>
                    <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: i <= currentStepIndex ? 'var(--color-primary)' : 'var(--color-muted)', fontWeight: i === currentStepIndex ? 700 : 400 }}>{stepLabel[s]}</span>
                  </div>
                  {i < STEPS.length - 1 && <div style={{ width: 40, height: 1, background: i < currentStepIndex ? 'var(--color-gold)' : '#E0D5C5', margin: '0 0.75rem' }} />}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="container" style={{ padding: '3rem 2rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '2.5rem', alignItems: 'start' }}>
            ...
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
              {step === 'shipping' && (
                <div>
                  <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 400, color: 'var(--color-primary)', marginBottom: '2rem' }}>Shipping Information</h2>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem' }}>
                    ...
                    <div style={{ flex: '1 1 calc(50% - 0.5rem)', minWidth: 120 }}>
                      <label style={{ fontFamily: 'var(--font-display)', fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-muted)', display: 'block', marginBottom: '0.4rem' }}>State</label>
                  ...
                  <button onClick={handleNext} className="btn btn-gold" style={{ marginTop: '2rem' }}>Continue to Payment →</button>
                </div>
              )}

              {step === 'payment' && (
                <div>
                  <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 400, color: 'var(--color-primary)', marginBottom: '2rem' }}>Payment Details</h2>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', marginTop: '2rem' }}>
                    <button onClick={() => setStep('shipping')} className="btn btn-outline">← Back</button>
                    <button onClick={handleNext} className="btn btn-gold">Review Order →</button>
                  </div>
                </div>
              )}
              ...
              {step === 'review' && (
                <div>
                  <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.5rem', fontWeight: 400, color: 'var(--color-primary)', marginBottom: '2rem' }}>Order Review</h2>
                  {/* Shipping summary */}
                  <div style={{ background: '#fff', border: '1px solid #EDE8DC', borderRadius: 6, padding: '1.25rem', marginBottom: '1.25rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--color-muted)' }}>Shipping To</h3>
                      <button onClick={() => setStep('shipping')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-display)', fontSize: '0.62rem', letterSpacing: '0.1em', color: 'var(--color-gold)', textTransform: 'uppercase' }}>Edit</button>
                    </div>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: 'var(--color-primary)' }}>{shipping.firstName} {shipping.lastName}</p>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', color: 'var(--color-muted)' }}>{shipping.address}, {shipping.city}, {shipping.state} — {shipping.pincode}</p>
                  </div>
                  {/* Payment summary */}
                  <div style={{ background: '#fff', border: '1px solid #EDE8DC', borderRadius: 6, padding: '1.25rem', marginBottom: '2rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--color-muted)' }}>Payment</h3>
                      <button onClick={() => setStep('payment')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-display)', fontSize: '0.62rem', letterSpacing: '0.1em', color: 'var(--color-gold)', textTransform: 'uppercase' }}>Edit</button>
                    </div>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: 'var(--color-primary)' }}>Ending in {payment.cardNumber.slice(-4)}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '1rem' }}>
                    <button onClick={() => setStep('payment')} className="btn btn-outline">← Back</button>
                    <button onClick={handlePlaceOrder} className="btn btn-gold" disabled={placing} style={{ opacity: placing ? 0.7 : 1 }}>
            ...
            {/* Order Summary */}
            <div style={{ background: '#fff', border: '1px solid #EDE8DC', borderRadius: 8, padding: '1.75rem', position: 'sticky', top: 92 }}>
              <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.25rem', fontWeight: 500, color: 'var(--color-primary)', marginBottom: '1.25rem' }}>Order Summary</h2>
              {items.map(item => (
                <div key={`${item.product.id}-${item.variant.sku}`} style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem', alignItems: 'center' }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <img src={item.product.images[0]} alt="" style={{ width: 52, height: 52, objectFit: 'cover', borderRadius: 4 }} />
                    <span style={{ position: 'absolute', top: -6, right: -6, background: 'var(--color-primary)', color: '#fff', borderRadius: '50%', width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', fontWeight: 700 }}>{item.quantity}</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.82rem', fontWeight: 500, color: 'var(--color-primary)' }}>{item.product.name}</p>
                    <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.72rem', color: 'var(--color-muted)' }}>{item.variant.size}{item.variant.unit}</p>
                  </div>
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-primary)' }}>₹{(item.variant.price * item.quantity).toLocaleString('en-IN')}</span>
                </div>
              ))}
              <div style={{ borderTop: '1px solid #EDE8DC', paddingTop: '1rem', marginTop: '0.5rem' }}>
                {[
                  ...
                  <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.6rem' }}>
                    <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.85rem', color: 'var(--color-muted)' }}>{row.label}</span>
                    <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', fontWeight: 600, color: 'var(--color-primary)' }}>{row.val}</span>
                  </div>
                ))}
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '0.75rem', borderTop: '1px solid #EDE8DC' }}>
                  <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--color-primary)' }}>Total</span>
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', fontWeight: 700, color: 'var(--color-primary)' }}>₹{total.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
          </div>
