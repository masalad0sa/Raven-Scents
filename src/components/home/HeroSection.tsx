...
export function HeroSection({ isMobile }: { isMobile: boolean }) {
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="container" style={{ position: "relative", zIndex: 4, padding: isMobile ? "6rem 1rem 4rem" : "8rem 2rem 6rem" }}>
        <motion.p
          variants={fadeUp}
          initial="hidden"
          animate="visible"
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: "3rem", flexWrap: "wrap" }}>
          <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={4} style={{ fontFamily: "var(--font-sans)", fontSize: "1rem", color: "var(--color-muted)", maxWidth: 320, lineHeight: 1.8 }}>
        ...
        <div className="container no-scrollbar" style={{ display: "flex", gap: "3rem", overflowX: "auto" }}>
          {[
            ...
            <div key={stat.label} style={{ flexShrink: 0 }}>
              <div style={{ fontFamily: "var(--font-serif)", fontSize: "1.5rem", fontWeight: 500, color: "var(--color-gold)" }}>
                {stat.value}
              </div>
              <div style={{ fontFamily: "var(--font-display)", fontSize: "0.6rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--color-muted)", marginTop: "0.2rem" }}>
                {stat.label}
              </div>
            </div>
          ))}
