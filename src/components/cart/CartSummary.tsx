...
export function CartSummary({ subtotal, shippingFee, total, discount, appliedCoupon, setAppliedCoupon }: Props) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
      style={{ background: "#1a1a1a", border: "1px solid rgba(212,175,55,0.15)", borderRadius: 8, padding: "1.75rem", position: "sticky", top: 92 }}
    >
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        {appliedCoupon ? (
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.75rem 1rem", background: "rgba(46, 204, 113, 0.08)", border: "1px solid rgba(46, 204, 113, 0.3)", borderRadius: 4 }}>
            ...
            <button onClick={() => { setAppliedCoupon(null); setCoupon(""); sessionStorage.removeItem("raven_coupon"); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-muted)", marginLeft: "auto" }}>✕</button>
          </div>
        ) : (
          <div style={{ display: "flex", gap: "0.5rem" }}>
            ...
            <button onClick={applyCoupon} disabled={couponLoading} className="btn btn-outline" style={{ padding: "0.75rem 1rem", whiteSpace: "nowrap" }}>Apply</button>
          </div>
        )}
      ...
      {/* Totals */}
      {[
        { label: "Subtotal", val: `₹${subtotal.toLocaleString("en-IN")}`, color: "var(--color-primary)" },
        { label: "Shipping", val: shippingFee === 0 ? "Free" : `₹${shippingFee}`, color: shippingFee === 0 ? "var(--color-success)" : "var(--color-primary)" },
        ...(appliedCoupon ? [{ label: `Discount (${appliedCoupon.pct}%)`, val: `-₹${discount.toLocaleString("en-IN")}`, color: "var(--color-success)" }] : []),
      ].map((row) => (
        <div key={row.label} style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.85rem" }}>
          ...
          <span style={{ fontFamily: "var(--font-display)", fontSize: "0.9rem", fontWeight: 600, color: row.color }}>{row.val}</span>
        </div>
      ))}
      ...
      <Link to="/checkout" className="btn btn-gold" style={{ width: "100%", justifyContent: "center", display: "flex", textDecoration: "none" }}>Proceed to Checkout →</Link>
