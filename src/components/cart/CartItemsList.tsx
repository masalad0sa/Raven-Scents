...
interface CartItem {
  product: { id: string; slug: string; images: string[]; name: string; brand: string };
...
export function CartItemsList({ items, isMobile, removeItem, updateQuantity }: Props) {
  return (
    <div>
      {!isMobile && (
      ...
      {items.map((item) => (
        <motion.div key={`${item.product.id}-${item.variant.sku}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          style={isMobile
            ? { display: "flex", flexDirection: "column", gap: "0.75rem", padding: "1rem 0", borderBottom: "1px solid rgba(212,175,55,0.15)" }
            : { display: "grid", gridTemplateColumns: "2.5fr 1fr 1fr 1fr auto", gap: "1rem", alignItems: "center", padding: "1.5rem 0", borderBottom: "1px solid rgba(212,175,55,0.15)" }
          }
        >
          {/* Product */}
          ...
          {isMobile ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <span style={{ fontFamily: "var(--font-display)", fontWeight: 700, color: "var(--color-gold)" }}>
            ...
            <>
              ...
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(212,175,55,0.5)", display: "flex", transition: "color 0.2s" }}
...
function QtyControl({ item, updateQuantity }: { item: CartItem; updateQuantity: (id: string, sku: string, qty: number) => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", border: "1px solid #333", borderRadius: 4, padding: "0.3rem 0.6rem", width: "fit-content" }}>
      <button onClick={() => updateQuantity(item.product.id, item.variant.sku, item.quantity - 1)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-primary)", display: "flex" }}>
        <Minus size={12} />
      </button>
      <span style={{ fontFamily: "var(--font-display)", fontWeight: 700, minWidth: 20, textAlign: "center", color: "var(--color-primary)", fontSize: "0.85rem" }}>
        {item.quantity}
      </span>
      <button onClick={() => updateQuantity(item.product.id, item.variant.sku, item.quantity + 1)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-primary)", display: "flex" }}>
        <Plus size={12} />
